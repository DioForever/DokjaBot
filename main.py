...
async def m(ctx, *args):
    ...
    if cmds.__contains__(args[0]):
        ...
        for manhwa in manhwas:
            ...
            if manhwa[2] == args[0]:
                ...
                if source == 'ReaperScans':
                    ...
                    embed = \
                        getLuminousScans(manhwa[3], manhwa[5], manhwa[6], int(manhwa[7]), int(manhwa[8]),
                                         int(manhwa[9]),
                                         int(manhwa[10]), int(manhwa[11]), int(manhwa[12]), str(id_guild))[0]
                    await ctx.send(embed=embed)
                elif source == 'MangaKakalot':
                    embed = \
                        getMangaKakalot(manhwa[3], manhwa[5], manhwa[6], int(manhwa[7]), int(manhwa[8]), int(manhwa[9]),
                                        int(manhwa[10]), int(manhwa[11]), int(manhwa[12]), str(id_guild))[0]
                    await ctx.send(embed=embed)
                else:
        ...
        await ctx.send(embed = embed)'''
        getMangaKakalotReleased("FFF Class Trashero", '', '', 1, 1, 1)
    elif args[0] == "help":
...
def getReaperScansReleased(Title, urlbasic, urlchapter, r1, g, b, id_channel, id_guild):
    ...
    try:
        ...
        url_thumbnail = 'https://cdn.discordapp.com/attachments/977231331199164466/979778575890780180/unknown.png'
